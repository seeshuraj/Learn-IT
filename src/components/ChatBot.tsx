...
import { askModuleChatbot, ChatMessage } from "../services/aiService";
...
export const ChatBot: React.FC<Props> = ({
  ...
  const send = async () => {
    ...
    setLoading(true);
    try {
      const history = messages.filter((m) => m.role !== "assistant" || m.content !== WELCOME);
        ...
        <div className="fixed bottom-6 right-6 w-80 sm:w-96 h-[520px] bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col z-50 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-semibold text-white leading-tight">AI Course Assistant</p>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-slate-100 px-3 py-2 rounded-2xl rounded-bl-md flex items-center gap-1.5">
          ...
          <div className="px-3 py-3 border-t border-slate-100 flex gap-2 items-end">
            <input
              ...
              disabled={loading}
              className="flex-1 text-sm border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:border-violet-400 focus:ring-1 focus:ring-violet-200 resize-none disabled:opacity-50 placeholder:text-slate-400"
            />
            <button
              onClick={send}
