...
export function AssessmentsPage({ role }: { role: "instructor" | "student" }) {
  ...
  async function fetchExams() {
    try {
      setLoading(true);
      const res = await fetch(`/api/instructor/courses/${courseId}/unit-exams`, {
        credentials: "include",
      });
  ...
  function bandColor(band: string) {
    if (band === "strong") return "text-green-600 bg-green-50";
    if (band === "moderate") return "text-amber-600 bg-amber-50";
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="space-y-3">
        {exams.map((exam) => {
          const pct =
            exam.avg_marks != null && exam.max_marks > 0
              ? Math.round((exam.avg_marks / exam.max_marks) * 100)
              : null;
            ...
            >
              ...
              {exam.total_results > 0 && (
                <div className="mt-3 h-1.5 rounded-full bg-gray-100 overflow-hidden flex">
                  <div
                    style={{ width: `${(exam.strong_count / exam.total_results) * 100}%` }}
                    className="bg-green-400"
                  />
                  <div
                    style={{ width: `${(exam.moderate_count / exam.total_results) * 100}%` }}
                    className="bg-amber-400"
                  />
                  <div
                    style={{ width: `${(exam.weak_count / exam.total_results) * 100}%` }}
